...
//To sign token to be sent to authenticated users
const signToken = (id) => {
...
//To send the signed token to authenticated user
const createSendToken = (user, statusCode, req, res) => {
  const token = signToken(user._id);
  //to define httpOnly cookie options
  const cookieOptions = {
    expires: new Date(
  ...
  //to ensure cookies are sent over secure connection (https)
...
//to send password reset token to users via email
exports.resetPassword = catchAsync(async (req, res, next) => {
  // 1) Get user based on token saved in the DB
  ...
  // 3) Update changedPasswordAt property for user. (This is implemented in the userModel)
